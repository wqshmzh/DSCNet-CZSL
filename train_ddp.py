...
from torch.distributed import barrier, init_process_group, reduce
...
def main(rank, config, **kwargs):
    ...
    config.train_batch_size = int(config.train_batch_size // config.gradient_accumulation_steps // config.world_size)
    
    print('max epochs: ', config.epochs)

    init_process_group(backend='nccl', init_method=f'tcp://localhost:{config.port}', world_size=config.world_size, rank=config.rank)
    ...
    lock = kwargs['lock']
    print_free = print_select(config.rank, restriction=False) # All ranks can use this func to print strings
    barrier()
    time.sleep(config.rank*0.1)
    lock.acquire()
    print_free(f'Choose device: {config.device} ({device_info.name}, {round(device_info.total_memory/1024**3, ndigits=2)}GB)')
    lock.release()
    barrier()
    ...
    val_dataset = CompositionDataset(dataset_path,
                                     ...
                                     print=print)
    train_sampler = DistributedSampler(train_dataset, seed=config.seed)
    train_batch_sampler = BatchSampler(train_sampler, batch_size=config.train_batch_size, drop_last=False)
    ...
    val_batch_sampler = BatchSampler(val_sampler, batch_size=config.eval_batch_size, drop_last=False)
    ...
    print(f'  total training batch size = {config.train_batch_size * config.world_size * config.gradient_accumulation_steps}')
...
def train_model(epoch, model, optimizer, scheduler, config, train_loader, train_losses, metrics, print, **kwargs):
    ...
    for bid in progress_bar:
        ...
        # In ddp, this loss does not require to be timed by config.world_size,
        # because the loss is averaged across instances in a batch, not summed
        ...
        if is_update:
            ...
            with model.no_sync():
                if config.use_amp:
                    ...
                    loss.backward()
        epoch_train_losses.append(loss.item() / config.world_size)
