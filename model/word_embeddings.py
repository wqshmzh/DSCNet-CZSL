...
def load_word_embeddings(vocab, config, print):
    if "glove" in config.emb_type:
        embeds = load_glove_embeddings(vocab, config.glove_path, print)
    elif "fasttext" in config.emb_type:
        embeds = load_fasttext_embeddings(vocab, config.fasttext_path, print)
    elif "word2vec" in config.emb_type:
        embeds = load_word2vec_embeddings(vocab, config.word2vec_path, print)
    elif "ft+w2v" in config.emb_type:
        embeds1 = load_fasttext_embeddings(vocab, config.fasttext_path, print)
        embeds2 = load_word2vec_embeddings(vocab, config.word2vec_path, print)
...
def load_fasttext_embeddings(vocab, file_path, print):
    custom_map = {
        'Faux.Fur': 'fake fur',
    ...
    ft = fasttext.load_model(file_path)
    embeds = []
    for k in vocab:
        if '_' in k:
            ks = k.split('_')
...
def load_word2vec_embeddings(vocab, file_path, print):
    # vocab = [v.lower() for v in vocab]

    from gensim import models
    model = models.KeyedVectors.load_word2vec_format(file_path, binary=True)
...
def load_glove_embeddings(vocab, file_path, print):
    '''
    Inputs
        file_path: Text file with word embedding pairs e.g. Glove, Processed in lower case.
        vocab: List of words
    Returns
        Embedding Matrix
    '''
