...
class CustomTextEncoder(torch.nn.Module):
    ...
    def forward(self, token_ids, token_tensors, enable_pos_emb):
        ...
        if token_tensors is not None:
            text_embeddings = token_tensors
        else:
            text_embeddings = self.token_embedding(token_ids)

        text_embeddings = text_embeddings.type(self.dtype)
        x = (
            text_embeddings + self.positional_embedding.type(self.dtype)
            if enable_pos_emb
            else text_embeddings
        )
        x = x.permute(1, 0, 2)
        text_features = self.transformer(x)

        x = text_features.permute(1, 0, 2)
        x = self.ln_final(x)
        tf = (
            x[
...
class SelfResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_dropout: float, mlp_dropout: float, attn_mask: torch.Tensor = None):
        super().__init__()

        self.attn = nn.MultiheadAttention(d_model, n_head, dropout=attn_dropout, batch_first=True)
        self.ln_1 = LayerNorm(d_model)
        self.mlp = nn.Sequential(OrderedDict([
            ("c_fc", nn.Linear(d_model, d_model * 4)),
            ("gelu", QuickGELU()),
            ("drop", nn.Dropout(mlp_dropout)),
